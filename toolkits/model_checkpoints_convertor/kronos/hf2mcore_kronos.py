...
from kronos.pretrain_kronos import model_provider, GPTModel
...
sys.path.append(os.path.join(path_dir, "kronos"))
from Model.kronos import KronosU
...
def check_hf_mg_forward(hfmodel: KronosU, mgmodel: GPTModel, mgargs):
    if mgargs.fp16:
        mgmodel = mgmodel.half()
        hfmodel = hfmodel.half()
    elif mgargs.bf16:
    ...
    def print_input_hook(module, args, kwargs, layer_idx, mode):
        frame, name = mode.split("-")
        if frame == "hf" and "attn_in" in mode:
            hf_hiddens[layer_idx][name] = args[0]
        elif frame == "hf":
            hf_hiddens[layer_idx][name] = args[0].transpose(0, 1)
        elif frame == "mg" and "layer" in mode:
            mg_hiddens[layer_idx][name] = kwargs.get("hidden_states")
    ...
    hfmodel.head.register_forward_hook(
        partial(print_output_hook, layer_idx=mgargs.num_layers - 1, mode="hf-lmhead"),
        with_kwargs=True,
    )
    ...
    for idx, layer in enumerate(hfmodel.transformer):
        layer.register_forward_pre_hook(
            partial(print_input_hook, layer_idx=idx, mode="hf-layer_in"),
            with_kwargs=True,
        )
        ...
        layer.norm2.register_forward_pre_hook(
            partial(print_input_hook, layer_idx=idx, mode="hf-pre_ln_in"),
            with_kwargs=True,
        )
        ...
        layer.ffn.w2.register_forward_pre_hook(
            partial(print_input_hook, layer_idx=idx, mode="hf-down_in"),
            with_kwargs=True,
        )
    ...
    with torch.inference_mode():
        try:
            hfmodel.cuda()
            hflogits = hfmodel(s_ids=input_ids)
        except torch.cuda.OutOfMemoryError:
            print("oom for huggingface model forward")
            is_oom = True
        hfmodel.cpu()
...
def convert_checkpoint_from_megatron_to_transformers(mgmodel: GPTModel, hfmodel: KronosU, args):
    if args.fp16:
        mgmodel = mgmodel.half()
        hfmodel = hfmodel.half()
    elif args.bf16:
    ...
    with torch.no_grad():
        # 1. embedding, hfmodel only have word_embeddings
        hfmodel.embedding.weight.copy_(
            mgmodel.embedding.word_embeddings.weight
        )
        ...
        if not args.no_rotary_embed_copy:
            ...
            for i in range(args.num_layers):
                hfmodel.transformer[i].self_attn.rotary.inv_freq.copy_(mgmodel.rotary_pos_emb.inv_freq)

        # 3. all decode layers
        for mglayer, hflayer in zip(mgmodel.decoder.layers, hfmodel.transformer):
            # 3.1. input layernorm (RMSNorm, no bias)
            if use_te:
                hflayer.norm1.weight.copy_(
                    mglayer.self_attention.linear_qkv.layer_norm_weight
                )
            else:
                hflayer.norm1.weight.copy_(mglayer.input_layernorm.weight)
            ...
            # 3.3 linear proj
            hflayer.self_attn.out_proj.weight.copy_(
                mglayer.self_attention.linear_proj.weight
            )
            ...
            hflayer.ffn.w1.weight.copy_(gate_weight)
            hflayer.ffn.w3.weight.copy_(fc1_weight)
            hflayer.ffn.w2.weight.copy_(mglayer.mlp.linear_fc2.weight)

            if use_te:
                hflayer.norm2.weight.copy_(
                    mglayer.mlp.linear_fc1.layer_norm_weight
                )
            else:
                hflayer.norm2.weight.copy_(mglayer.pre_mlp_layernorm.weight)

        hfmodel.norm.weight.copy_(mgmodel.final_layernorm.weight)
        hfmodel.head.weight.copy_(mgmodel.output_layer.weight)
...
def main():
    ...
    if args.convert_checkpoint_from_megatron_to_transformers:
        ...
        hf_model.save_pretrained(args.save)
    else:
